...
describe("Bank Contract", function () {
  ...
  let addr2; // This will hold the second test account.
  ...
  it("should allow a user to deposit funds and update their balance", async function () {
    ...
    // Set the deposit amount to 2 Ether.
    // 'ethers.utils.parseEther("2.0")' converts the amount from Ether to Wei (1 Ether = 10^18 Wei).
    const depositAmount = ethers.utils.parseEther("2.0");

    // Deposit 2 Ether from 'addr1' into the bank contract.
    // 'bank.connect(addr1).depositFunds({ value: depositAmount })' sends a transaction to the depositFunds function with 2 Ether from addr1.
  ...
  it("should allow a user to withdraw funds and update their balance", async function () {
    // Set the deposit amount to 2 Ether and deposit it.
    const depositAmount = ethers.utils.parseEther("2.0");
    await bank.connect(addr1).depositFunds({ value: depositAmount });

    // Set the withdraw amount to 1 Ether.
    const withdrawAmount = ethers.utils.parseEther("1.0");

    // Withdraw 1 Ether from 'addr1's balance.
    const tx = await bank.connect(addr1).withdrawFunds(withdrawAmount);
    await tx.wait(); // Wait for the transaction to be mined.

    // Check the balance of 'addr1' after the withdrawal.
    const balance = await bank.connect(addr1).balanceCheck();

    // Expect the balance to be the deposit amount minus the withdraw amount.
  ...
  it("should not allow withdrawing more Ether than the user's balance", async function () {
    // Expect the withdrawal of 1 Ether to be reverted with an error message due to insufficient balance.
    await expect(
      bank.connect(addr1).withdrawFunds(ethers.utils.parseEther("1.0"))
