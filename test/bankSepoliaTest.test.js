...
describe("Bank Contract on Sepolia Network", function () {
  ...
  beforeEach(async function () {
    ...
    // Create a provider for Sepolia network using Alchemy.
    // 'ethers.providers.JsonRpcProvider' connects to an Ethereum node using a specific URL.
    const provider = new ethers.providers.JsonRpcProvider(
      `https://sepolia.infura.io/v3/${process.env.INFURA_PROJECT_ID}`
    );

    // Use pre-defined accounts from environment variables with the provider.
    // 'new ethers.Wallet' creates a new instance of a wallet with a private key and connects it to the provider.
    owner = new ethers.Wallet(process.env.PRIVATE_KEY_OWNER, provider);
    addr1 = new ethers.Wallet(process.env.PRIVATE_KEY_ADDR1, provider);

    // Deploy a new instance of the 'Bank' contract using the owner's account.
    // 'Bank.connect(owner).deploy()' deploys a new instance of the contract connected with the owner's account.
  ...
  it("should allow a user to withdraw funds and update their balance", async function () {
    ...
    console.log("Attempting to withdraw:", withdrawAmount.toString()); // Print the withdraw amount for debugging.
